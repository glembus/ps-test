...
final class CommissionFeeService
{
    ...
    public function calculateCommissionFee(TransactionInterface $transaction): Fee
    {
        ...
        $baseRate = $this->exchangeService->getExchangeRate($transaction->getCurrency())->getBackwardExchangeRate('EUR');
        $fee = new Fee($transaction, $baseRate, $statistics);
        $feeCalculator->calculateFee($fee);
        $this->transactionStorage->updateUserWeekTransactionsStatistic($transaction->getUserId(), $fee->getUserWeekTransactionStatistic());
